...
## Files
- **comavi_v7_results.csv** — output of Stages 1-6 (modular `pipeline.run_pipeline`).
  Contains structural features, FoldX three-axis ΔΔG values, P1 scoring,
  comavi_tier, comavi_mechanism. ~262 columns × 44 rows.
- **comavi_v7_results_corrected.csv** — output of Stage 7 (`baseline_correct.py`).
  Adds per-(system, partner) baseline-indistinguishable flags + reclassified
  mechanism. **Track A consumes this.**
- **comavi_v7_results_with_nbhd.csv** — output of Stage 8 (`neighborhood.py`).
...
2. Stage 7:   `python3 scripts/comavi_v7_baseline_correct.py` (~5 sec)
3. Stage 8:   `python3 archive/derivation/comavi_v7_neighborhood.py` (~30 sec, needs structures)
...
- After applying the patches, these files will get NEW columns: `comavi_mechanism_t10/t15/t20/t25` and `comavi_mechanism_corrected_t10/t15/t20/t25`
