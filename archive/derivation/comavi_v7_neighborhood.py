#!/usr/bin/env python3
"""
COMAVI v7 — Neighborhood (Pipeline 2) extraction.

Ports the ±3 neighborhood contact extraction from CHD_pipeline_v6_0_A3-6
Cells 4 and 5 to COMAVI v7. Produces a second set of structural footprint
...
-----
    cd ~/comavi_v7
    python3 comavi_v7_neighborhood.py

Reads:   results/comavi_v7_results_corrected.csv + structure files
Writes:  results/comavi_v7_results_with_nbhd.csv

Requires the comavi_v7 package importable from the working directory (i.e.
run from ~/comavi_v7/ or with PYTHONPATH set).
"""
...
# comavi_v7 package imports — expected to be importable from CWD
try:
    from comavi_v7.config import build_benchmark_config
    from comavi_v7.structure_loading import (
        load_monomer, load_multimer, get_plddt,
        count_intra_contacts, count_interface,
    )
    from comavi_v7.metrics import collect_partner_labels, _partner_label_for_chain
except ImportError as e:
    print(f"ERROR: could not import comavi_v7 package: {e}", file=sys.stderr)
    print("       Run this script from ~/comavi_v7/ (with comavi_v7/ as a subdirectory)", file=sys.stderr)
    sys.exit(1)
...
def main():
    ...
    ap.add_argument("--input",        default="results/comavi_v7_results_corrected.csv")
    ap.add_argument("--output",       default="results/comavi_v7_results_with_nbhd.csv")
    ...
    ap.add_argument("--config",       default="comavi_v7/system_configs.yaml",
                    help="System config file (YAML). Default matches comavi_v7 layout.")
    args = ap.parse_args()
