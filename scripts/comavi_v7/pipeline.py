...
def run_pipeline(
    ...
    # Default `comavi_mechanism` (no suffix) aliases the t10 column for
    ...
    for tag, thr in threshold_specs:
        ...
        df[f"comavi_mechanism_{tag}"] = [r[0] for r in mech_results]
        df[f"comavi_mechanism_partner_{tag}"] = [r[1] for r in mech_results]
        df[f"comavi_external_evidence_flag_{tag}"] = [r[2] for r in mech_results]

    # Backward-compatible aliases at t=1.0 (the historical default)
    df["comavi_mechanism"] = df["comavi_mechanism_t10"]
    df["comavi_mechanism_partner"] = df["comavi_mechanism_partner_t10"]
    ...
    if verbose:
        ...
        print(df["comavi_mechanism_tSAP"].value_counts().to_string())
        print(f"\n  Tier distribution:")
        print(df["comavi_tier"].value_counts().to_string())
