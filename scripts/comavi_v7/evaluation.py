"""
COMAVI v7 — Phase 3 evaluation module (revised).
...
Primary threshold: 1.0 kcal/mol (matches the COMAVI pipeline's production
...
# Level 1 classifiers
# ============================================================================
def comavi_full_positive(row, cfg: ClassifierConfig) -> bool:
...
def structural_score_only_positive(row, cfg: ClassifierConfig) -> bool:
    tier = str(row.get("comavi_tier", "")).strip()
...
def level1_single_threshold(
    ...
    classifiers = {
        "COMAVI_full": comavi_full_positive,
...
def is_structural_disruption_detected(row, ddg_threshold: float = 1.0) -> bool:
    """
    v5: detection = (comavi_tier in {Tier 1, Tier 2}) OR any DDG axis fires.
    Convention-independent: doesn't care about destab vs stab direction,
    just whether COMAVI produced a non-silent prediction.

    Args:
      row: pd.Series with comavi_tier and ddg_* columns
      ddg_threshold: scalar threshold (a single value applied to all axes)
    """
    if str(row.get("comavi_tier")) in FOOTPRINT_TIERS:
        return True
    # Monomer
...
def level4_gof_detection(
    ...
    for _, r in gofs.iterrows():
        ...
        for t in thresholds:
            ...
            tkey = f"t{int(t*10):02d}"
            row_info[f"comavi_positive_{tkey}"] = bool(comavi_full_positive(r, cfg))
        ...
        for mech_col in ("comavi_mechanism_corrected_t10", "comavi_mechanism_corrected_t15"):
    ...
    for t in thresholds:
        tkey = f"t{int(t*10):02d}"
        comavi_hits = int(per_variant_df[f"comavi_positive_{tkey}"].sum())
        ...
        summary_rows.append({
            ...
            "n_gof_total": n_gof,
            "COMAVI_full_hits": comavi_hits,
            "COMAVI_full_recall": round(comavi_hits / n_gof, 3),
