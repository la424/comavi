...
into the comavi_v7 package conventions.

DESIGN (Path A, scope-fenced):
  - This is a COLUMN-CONSUMING voting layer. It computes nothing structural.
  - It reads engine outputs (comavi_tier, per-axis ddg + confidence) plus EXTERNAL
...
def compute_concordance(row, partner_labels: List[str]) -> pd.Series:
    ...
    """
    tier = ss(row.get('comavi_tier'))                  # package col (was v6_tier)
