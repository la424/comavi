"""
COMAVI v7 baseline-correction post-processor.
...
Usage:
  cd ~/comavi_v7
  python3 comavi_v7_baseline_correct.py

Writes:
  results/comavi_v7_results_corrected.csv  (with baseline flags + new mechanism)
...
from comavi_v7.mechanism import classify_mechanism
...
def reclassify_with_flags(df: pd.DataFrame, partner_labels: list) -> pd.DataFrame:
    ...
    v5 thresholds. Writes one set of corrected columns per threshold:
      - comavi_mechanism_corrected_t10
      - comavi_mechanism_corrected_t15
      - comavi_mechanism_corrected_t20
      - comavi_mechanism_corrected_t25
      - comavi_mechanism_corrected_tSAP

    The legacy column `comavi_mechanism_corrected` (no suffix) is kept as an
    ...
    for tag, thr in threshold_specs:
        ...
        new_cols_dict[f"comavi_mechanism_corrected_{tag}"] = mechs
        new_cols_dict[f"comavi_mechanism_partner_corrected_{tag}"] = partners
        new_cols_dict[f"comavi_external_evidence_flag_corrected_{tag}"] = exts

    # Backward-compatible aliases
    new_cols_dict["comavi_mechanism_corrected"] = new_cols_dict["comavi_mechanism_corrected_t10"]
    new_cols_dict["comavi_mechanism_partner_corrected"] = new_cols_dict["comavi_mechanism_partner_corrected_t10"]
    new_cols_dict["comavi_external_evidence_flag_corrected"] = new_cols_dict["comavi_external_evidence_flag_corrected_t10"]
...
# ============================================================================
def main():
    results_path = Path("results/comavi_v7_results.csv")
    ...
    print(df["comavi_mechanism_corrected"].value_counts().to_string())

    # Diff: variants where mechanism CHANGED
    changed = df[df["comavi_mechanism"] != df["comavi_mechanism_corrected"]]
    print(f"\n  {len(changed)} variants had mechanism change after correction:")
    for _, r in changed.iterrows():
        print(f"    {r['system']}/{r['variant']}: "
              f"'{r['comavi_mechanism']}' → '{r['comavi_mechanism_corrected']}'")

    out_path = Path("results/comavi_v7_results_corrected.csv")
