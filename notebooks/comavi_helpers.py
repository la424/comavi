"""
comavi_helpers.py — pure-Python logic behind the COMAVI guided Colab notebook.
...
def resolve_uniprot(gene, organism_id=9606, reviewed=True, timeout=30):
    ...
    req = urllib.request.Request(
        url, headers={"User-Agent": "COMAVI-colab/1.0 (+https://github.com/la424/comavi)"})
...
def validate_config(yaml_path, scripts_dir):
    ...
    from comavi_v7.config_io import load_config
...
def _min_tier(g):
    import numpy as np
    if "comavi_tier" not in g:
        return float("nan")
    nums = [t for t in (_tier_num(v) for v in g["comavi_tier"])
...
def _mech_for_partner(g, partner):
    ...
    for _, row in g.iterrows():
        mech = row.get("comavi_mechanism")
        if mech is None or isinstance(mech, float):
            continue
        rp = row.get("comavi_mechanism_partner")
...
def overall_mechanism(g):
    ...
    for v in g.get("comavi_mechanism", []):
...
def run_concordance(structural_csv, annotations_df, outdir, scripts_dir, python_exe="python"):
    ...
    master = outdir / "comavi_v7_concordance.csv"
